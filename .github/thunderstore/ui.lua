-- CG3H in-game UI — ImGui mod manager (read-only, v3.7).
--
-- Swap this file to change the look.  It reads mod_state (data) and the
-- context table (for paths + refresh callbacks).  It never mutates game
-- state directly — every action goes through the context's callbacks so
-- this file stays decoupled from runtime.lua.

local M = {}

local ImGui = rom.ImGui
local window_open = false
local selected_mod_id = nil

-- Status → (icon, color).  Kept tiny so adding new states later (e.g.
-- v3.8 disabled/queued) is a one-line change.
local STATE_DECOR = {
...
local function _draw_characters_tab(state)
    ImGui.TextDisabled("Per-character build status. Click a header to expand.")
    ImGui.Separator()

    -- Deterministic order: sort characters alphabetically
    local chars = {}
    ...
    for _, char in ipairs(chars) do
        ...
        if ImGui.CollapsingHeader(label) then
            ...
            for _, mod in ipairs(mods) do
                ImGui.BulletText(string.format("%s  %s  by %s",
...
local function _draw_mods_tab(state)
    ...
    if ImGui.BeginTable("cg3h_mods_table", 5) then
        ...
        for _, mod in ipairs(state.mods) do
            ImGui.TableNextRow()
            ImGui.TableNextColumn(); ImGui.Text(mod.name ~= "" and mod.name or mod.id)
            ImGui.TableNextColumn(); ImGui.Text(mod.author ~= "" and mod.author or "-")
            ImGui.TableNextColumn(); ImGui.Text(mod.version ~= "" and mod.version or "-")
            ImGui.TableNextColumn(); ImGui.Text(mod.character)
...
local function _draw_window(state, ctx)
    -- H2M's Begin(name, open) returns (new_open, should_draw).  new_open
    -- reflects the X close button; should_draw is "visible AND not
    -- collapsed".  The binding only calls ImGui::Begin when open==true on
    -- entry, so we only call End in that same case — pairing stays clean.
...
-- @param ctx     { on_refresh = function() end }
function M.init(state, ctx)
    ctx = ctx or {}

    rom.gui.add_to_menu_bar(function()
        -- add_to_menu_bar already wraps this callback inside the plugin's
        -- own submenu (CG3HBuilder), so we emit the MenuItem at the top
        -- level — the final path is "CG3HBuilder -> Mod Manager", not
        -- "CG3HBuilder -> CG3H -> Mod Manager".
